...
rideRouter.put('/update-ride/:rideId', async (req, res) => {
  try {
    ...
    if (!ride) {
      return res.status(404).json({ error: 'Ride not found or you do not have permission to update this ride' });
    }

    // Update the ride details if found and permission is verified
    ride.from = from ? { latitude: from.latitude, longitude: from.longitude } : ride.from;
    ride.to = to ? { latitude: to.latitude, longitude: to.longitude } : ride.to;
    ride.available_seat = available_seat !== undefined ? available_seat : ride.available_seat;
    ride.amount_per_seat = amount_per_seat !== undefined ? amount_per_seat : ride.amount_per_seat;
